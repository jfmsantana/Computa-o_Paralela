...
int main(int argc, char **argv)
{
    ...
    {
        // MASTER
        int num_requisicoes = 200 + rand() % 51; // deixar mais pesado
        Requisicao *reqs = malloc(num_requisicoes * sizeof(Requisicao));

        // gera requisições grandes
        for (int i = 0; i < num_requisicoes; i++)
        {
            int chance = rand() % 10; // Gera número de 0 a 9 aleatoriamente

            if (chance < 5)
            {                                         // 50% de chance para a tarefa pesada
                reqs[i].op = 3;                       // operação 1..4 (forçado para ordenação)
                reqs[i].size = 55000 + rand() % 5001; // 2500 a 3000 elementos (vetor gigante)
            }
            else
            { // 50% de chance para uma tarefa leve aleatória

                int op_leve = rand() % 3; // Gera 0, 1 ou 2
                if (op_leve == 0)
                {
                    reqs[i].op = 1; // Média
                }
                ...
                {
                    reqs[i].op = 4; // Desvio Padrão
                }

                reqs[i].size = 20000 + rand() % 5001;
            }
        ...
        while (tarefas_concluidas < num_requisicoes)
        {
            ...
            if (tag == 1 || tag == 4)
            {
                ...
                if (tag == 1)
                    printf("Slave %d concluiu média -> %f\n", slave, result);
                else
                    printf("Slave %d concluiu desvio -> %f\n", slave, result);
            }
            else
            {
                int recv_values[MAX_NUMBERS];
                int recv_count;
                MPI_Recv(recv_values, MAX_NUMBERS, MPI_INT, slave, tag, MPI_COMM_WORLD, &status);
